...
export class ProductRecords implements ProductEntity {
  ...
  static async getRecentlyIssuedProducts(): Promise<ProductRecords[]> {
    const [results] = (await pool.execute(
      "SELECT `products`.*, MAX(`goods_issue`.`date`) as `date` FROM `products` JOIN `goods_issue` ON `products`.`id` = `goods_issue`.`productId` group by `products`.`id` ORDER BY `date` DESC LIMIT 25"
    )) as ProductRecordsResult;
  ...
  static async getRecentlyOrderedProducts(): Promise<ProductRecords[]> {
    const [results] = (await pool.execute(
      "SELECT `products`.*, MAX(`goods_reception`.`date`) as `date` FROM `products` JOIN `goods_reception` ON `products`.`id` = `goods_reception`.`productId` group by `products`.`id` ORDER BY `date` DESC LIMIT 25"
    )) as ProductRecordsResult;
